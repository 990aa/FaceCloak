...
Universal Adversarial Cloak includes explicit black-box transfer benchmarking against ArcFace and CLIP ViT-L/14, but transferability still depends on data domain, preprocessing differences, and defense pipelines in real systems.

**Dual-Use Nature**: Adversarial machine learning is a dual-use technology. The same equations that give individuals privacy against non-consensual surveillance can be used by malicious actors to bypass deepfake detection or evade legitimate biometric security checkpoints. Universal Adversarial Cloak is published as an open-source demonstration to democratize understanding of these vulnerabilities and advocate for algorithmic privacy.
